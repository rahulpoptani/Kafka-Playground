...
public class UserDataProducer {
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        // Note - .get() is used for testing only, do not use in prod
        ...
        // 4 - we send a user purchase for Rahul, but it exists in Kafka later
        System.out.println("\nExample 4 - non existing user then user\n");
        producer.send(purchaseRecord("Rahul", "Computer (4)")).get();
        producer.send(userRecord("Rahul", "First=Rahul,Last=Maarek,GitHub=rahulpoptani")).get();
        producer.send(purchaseRecord("Rahul", "Books (4)")).get();
        producer.send(userRecord("Rahul", null)).get(); // delete for cleanup
